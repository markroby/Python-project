...
# Define a function to handle missing values in specified columns of a dataframe
def handle_missing_values(df, columns, fill_value=''):
    for column in columns:
        df[column] = df[column].fillna(fill_value)
    return df

# Define a function to change the date format in specified columns of a dataframe
...
fig = px.line(df_fgroup, x="Date", y=["Confirmed", "Deaths", "Recovered", "Active"], 
              title='Analysis of Corona spread over time globally', 
              color_discrete_sequence=['#590d22', '#c9184a', '#ff758f', '#fff0f3'])
fig.show()
...
fig = px.bar(who.sort_values('Confirmed'), x='Confirmed', y="WHO Region", color='WHO Region', 
             text='Confirmed', orientation='h', width=700, color_discrete_sequence=px.colors.sequential.Aggrnyl)
fig.update_layout(title='Confirmed Cases by WHO Region', xaxis_title="", yaxis_title="", yaxis_categoryorder='total ascending')
fig.show()
